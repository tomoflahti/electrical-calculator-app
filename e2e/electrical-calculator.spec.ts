...
test.describe('Electrical Calculator End-to-End Tests', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/');
  });

  test('should load the main calculator page', async ({ page }) => {
    await expect(page).toHaveTitle(/Electrical/);
    await expect(page.getByRole('heading', { name: /electrical.*calculator/i })).toBeVisible();
  });
  ...
  test('should perform basic wire size calculation', async ({ page }) => {
    ...
    await page.fill('input[name="current"], input[aria-label*="Current"], input[label*="Current"]', '20');
    await page.fill('input[name="length"], input[aria-label*="Length"], input[label*="Length"]', '100');
    await page.fill('input[name="voltage"], input[aria-label*="Voltage"], input[label*="Voltage"]', '120');

    // Select voltage system if available
    const voltageSelect = page.locator('select[name="voltageSystem"], [aria-label*="Voltage System"]').first();
    ...
    // Check for results
    await expect(page.getByText(/recommended/i)).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should display voltage drop analysis', async ({ page }) => {
    ...
    await page.fill('input[aria-label*="Load Current"], input[label*="Current"]', '32');
    await page.fill('input[aria-label*="Circuit Length"], input[label*="Length"]', '50');
    await page.fill('input[aria-label*="Voltage"]', '400');

    // Look for chart or results
    const chartContainer = page.locator('[data-testid="voltage-drop-chart"], .recharts-wrapper, svg');
  ...
  test('should perform conduit fill calculation', async ({ page }) => {
    ...
    const wireGaugeInputs = page.locator('input[aria-label*="Gauge"], input[label*="Gauge"]');
    if (await wireGaugeInputs.first().isVisible()) {
      await wireGaugeInputs.first().fill('4');
    }

    const wireCountInputs = page.locator('input[aria-label*="Count"], input[label*="Count"]');
    if (await wireCountInputs.first().isVisible()) {
      await wireCountInputs.first().fill('3');
    }

    // Check for conduit recommendations
    await expect(page.getByText(/conduit.*size|recommended/i)).toBeVisible({ timeout: 10000 });
  });

  test('should handle standard switching (NEC/IEC)', async ({ page }) => {
    // Look for standard selector
    const standardSelector = page.locator('select[name="standard"], [aria-label*="Standard"]');
  ...
  test('should display charts when available', async ({ page }) => {
    ...
    const charts = page.locator('.recharts-wrapper, svg[class*="recharts"], [data-testid*="chart"]');
    ...
    if (chartCount > 0) {
      ...
      const chartSelectButton = page.getByRole('button', { name: /chart|view/i });
  ...
  test('should handle calculation errors gracefully', async ({ page }) => {
    ...
    const currentInput = page.locator('input[aria-label*="Current"], input[label*="Current"]').first();
  ...
  test('should maintain state during navigation', async ({ page }) => {
    ...
    const lengthInput = page.locator('input[aria-label*="Length"], input[label*="Length"]').first();
