...
export default function ConduitFillCalculator({ selectedStandard = 'NEC' }: ConduitFillCalculatorProps) {
  ...
  const [conduitType, setConduitType] = useState<string>(getDefaultConduitType(selectedStandard));
  const [wires, setWires] = useState<WireEntry[]>([getDefaultWire(selectedStandard)]);
  const [result, setResult] = useState<ConduitFillResult | null>(null);
  const [enhancedResult, setEnhancedResult] = useState<ConduitFillCalculationResult | null>(null);
  const [error, setError] = useState<string>('');
  const [prevStandard, setPrevStandard] = useState<ElectricalStandardId>(selectedStandard);
  
  // Enhanced functionality state
  const [applicationType, setApplicationType] = useState<ConduitFillApplicationType>('residential');
  const [installationMethod, setInstallationMethod] = useState<ConduitFillInstallationMethod>('indoor');
  ...
  const handleCalculate = () => {
    try {
      ...
      const input: ConduitFillCalculationInput = {
        ...
        installationMethod,
        ambientTemperature,
        environment: environment as 'indoor' | 'outdoor' | 'wet' | 'dry' | 'corrosive',
        futureFillReserve
      };
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" color="text.secondary" paragraph>
        Calculate required conduit size based on wire fill requirements per {standardName} standards.
      </Typography>
                ...
                <Grid item xs={12} sm={6}>
                  <FormControl fullWidth>
                    ...
                    <Select
                      ...
                      onChange={(e) => setApplicationType(e.target.value as ConduitFillApplicationType)}
                    ...
                    >
                      ...
                      <MenuItem value="high_temperature">High Temperature</MenuItem>
                    </Select>
                  </FormControl>
                </Grid>
              </Grid>
                ...
                <Grid item xs={12} sm={6}>
                  <FormControl fullWidth>
                    ...
                    <Select
                      ...
                      onChange={(e) => setInstallationMethod(e.target.value as ConduitFillInstallationMethod)}
                    ...
                    >
                      ...
                      <MenuItem value="corrosive">Corrosive Environment</MenuItem>
                      <MenuItem value="high_temperature">High Temperature</MenuItem>
                ...
                <Grid container spacing={2} sx={{ mb: 3 }}>
                  ...
                  <Grid item xs={12} sm={3}>
                    <TextField
                      ...
                      onChange={(e) => setFutureFillReserve(Number(e.target.value))}
                      size="small"
                    ...
                    <TableRow key={wire.id}>
                      <TableCell>
                        <FormControl size="small" sx={{ minWidth: 80 }}>
                          <Select
                            value={wire.gauge}
                            onChange={(e) => updateWire(wire.id, 'gauge', e.target.value)}
                      ...
                      <TableCell>
                        <TextField
                          ...
                          type="number"
                          value={wire.quantity}
                          onChange={(e) => updateWire(wire.id, 'quantity', Number(e.target.value))}
                          sx={{ width: 80 }}
                      ...
                      <TableCell>
                        <FormControl size="small" sx={{ minWidth: 80 }}>
                          <Select
                            ...
                            onChange={(e) => updateWire(wire.id, 'insulation', e.target.value)}
                ...
                <Box>
                  <Grid container spacing={2}>
                    <Grid item xs={12}>
                      <Box display="flex" alignItems="center" gap={1}>
                        <Typography variant="h4" color="primary">
                          {result.recommendedConduitSize}{selectedStandard === 'IEC' ? 'mm' : '"'}
                        </Typography>
                        <Chip
                      ...
                      <Typography variant="h6">
                        ...
                        {enhancedResult && (
                          <Typography variant="body2" color="text.secondary" component="span">
                            {' '}(max: {enhancedResult.fillAnalysis.maxAllowedFillPercentage}% - {enhancedResult.fillAnalysis.fillRule})
                          </Typography>
                        )}
                      </Typography>
                    </Grid>
                    ...
                    <Grid item xs={12}>
                      ...
                      <Typography variant="h6">
                        {result.totalWireArea.toFixed(4)} {selectedStandard === 'IEC' ? 'mm²' : 'sq in'}
                      </Typography>
                    </Grid>
                  </Grid>
                  ...
                  {enhancedResult && (
                    <Box mt={3}>
                      ...
                      <Grid container spacing={1} sx={{ mb: 2 }}>
                        <Grid item>
                          <Chip
                            icon={enhancedResult.compliance.codeCompliant ? <CheckCircle /> : <Error />}
                            label={`Code: ${enhancedResult.compliance.codeCompliant ? 'Pass' : 'Fail'}`}
                            color={enhancedResult.compliance.codeCompliant ? "success" : "error"}
                            size="small"
                          />
                        </Grid>
                        <Grid item>
                          <Chip
                            icon={enhancedResult.compliance.temperatureCompliant ? <CheckCircle /> : <Error />}
                            label={`Temperature: ${enhancedResult.compliance.temperatureCompliant ? 'Pass' : 'Fail'}`}
                            color={enhancedResult.compliance.temperatureCompliant ? "success" : "error"}
                            size="small"
                          />
                        </Grid>
                        <Grid item>
                          <Chip
                            icon={enhancedResult.compliance.applicationCompliant ? <CheckCircle /> : <Error />}
                            label={`Application: ${enhancedResult.compliance.applicationCompliant ? 'Pass' : 'Fail'}`}
                            color={enhancedResult.compliance.applicationCompliant ? "success" : "error"}
                            size="small"
                          />
                        </Grid>
                      </Grid>
                      ...
                      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                        <strong>Application:</strong> {enhancedResult.applicationData.applicationType} - {enhancedResult.applicationData.installationMethod}
                      </Typography>
                      
                      {enhancedResult.applicationData.specialRequirements && enhancedResult.applicationData.specialRequirements.length > 0 && (
                        <Box sx={{ mb: 2 }}>
                          <Typography variant="body2" color="text.secondary">
                            <strong>Special Requirements:</strong>
                          </Typography>
                          <ul style={{ margin: 0, paddingLeft: 20 }}>
                            {enhancedResult.applicationData.specialRequirements?.slice(0, 3).map((req, idx) => (
                              <li key={idx}>
                                <Typography variant="body2" color="text.secondary">
                                  {req}
                                </Typography>
                  ...
                  <Box mt={3}>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {selectedStandard === 'IEC' ? (
                        <><strong>IEC 60364 Guidelines:</strong><br />
                        • Maximum 40% fill for multiple conductors<br />
                        • Reduced fill for high temperature applications<br />
                        • Consider cable grouping factors</>
                      ) : (
                        <><strong>NEC Chapter 9 Guidelines:</strong><br />
                        • Maximum 40% fill for 3 or more conductors<br />
                        • Maximum 31% fill for 2 conductors<br />
                        • Maximum 53% fill for 1 conductor</>
                      )}
                    </Typography>
                  </Box>
                </Box>
