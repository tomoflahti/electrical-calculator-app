...
import { Public, Flag, Engineering, DirectionsCar, Sailing, WbSunny, Router } from '@mui/icons-material';
import { getAllStandards, getACStandards, getDCStandards, isDCStandard } from '../standards';
...
export default function StandardSelector({ selectedStandard, onStandardChange }: StandardSelectorProps) {
  const acStandards = getACStandards();
  const dcStandards = getDCStandards();
  const currentStandard = getAllStandards().find(s => s.id === selectedStandard);
  const isCurrentDC = isDCStandard(selectedStandard);

  const getStandardIcon = (standardId: ElectricalStandardId) => {
    switch (standardId) {
      case 'DC_AUTOMOTIVE': return DirectionsCar;
      case 'DC_MARINE': return Sailing;
      case 'DC_SOLAR': return WbSunny;
      case 'DC_TELECOM': return Router;
      default: return Flag;
    }
  };
      ...
      <CardContent>
        <Box display="flex" alignItems="center" gap={1} mb={2}>
          ...
          <Typography variant="h6">
            Electrical Standard Selection
          </Typography>
        </Box>
        
        <FormControl fullWidth sx={{ mb: 3 }}>
          <InputLabel>Electrical Standard</InputLabel>
          <Select
            value={selectedStandard}
            label="Electrical Standard"
            onChange={(e) => onStandardChange(e.target.value as ElectricalStandardId)}
          >
            {/* AC Standards Group */}
