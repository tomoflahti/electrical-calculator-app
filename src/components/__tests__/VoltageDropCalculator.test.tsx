...
const VoltageDropCalculatorWithStandard: React.FC<VoltageDropCalculatorProps> = ({ 
...
describe('VoltageDropCalculator', () => {
  ...
  describe('Standard Switching - Units and Labels', () => {
    ...
    it('should show mm² wire sizes for IEC standard', async () => {
      ...
      const wireGaugeSelect = screen.getByLabelText('Cable Size');
  ...
  describe('Calculation Function Switching', () => {
    it('should use NEC calculation for NEC standard', async () => {
      ...
      fireEvent.change(screen.getByLabelText('Length (ft)'), { target: { value: '100' } });
    ...
    it('should use IEC calculation for IEC standard', async () => {
      const { calculateVoltageDropAnalysis } = require('../../utils/calculations/iec');
      ...
      // Fill in some values and calculate
      fireEvent.change(screen.getByLabelText('Current (A)'), { target: { value: '32' } });
      fireEvent.change(screen.getByLabelText('Length (m)'), { target: { value: '50' } });
  ...
  describe('Results Display', () => {
    it('should show NEC compliance messages for NEC standard', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/Branch circuits: ≤3% voltage drop/)).toBeInTheDocument();
      });
    });
    ...
    it('should show IEC compliance messages for IEC standard', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/Final circuits: ≤4% voltage drop/)).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Unit Conversion', () => {
    it('should convert length from feet to meters when switching from NEC to IEC', () => {
      ...
      const lengthInput = screen.getByLabelText('Length (m)') as HTMLInputElement;
    ...
    it('should convert length from meters to feet when switching from IEC to NEC', () => {
      ...
      const lengthInput = screen.getByLabelText('Length (ft)') as HTMLInputElement;
  ...
  describe('Input Validation', () => {
    ...
    it('should maintain input validity when switching standards', () => {
      ...
      const currentInput = screen.getByLabelText('Current (A)') as HTMLInputElement;
  ...
  describe('Error Handling', () => {
    it('should handle calculation errors gracefully', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Invalid input parameters/)).toBeInTheDocument();
      });
    });
