...
const NECWireAmpacityChart: React.FC<NECWireAmpacityChartProps> = ({
  ...
  const chartData = useMemo(() => {
    const materialMultiplier =
      material === "aluminum"
        ? NEC_CONDUCTOR_MATERIALS.aluminum.multiplier
        : 1.0;
    const adjustmentFactor =
      NEC_CONDUCTOR_ADJUSTMENT_FACTORS[
        selectedConductorCount as keyof typeof NEC_CONDUCTOR_ADJUSTMENT_FACTORS
      ] || 1.0;

    // Get temperature correction factor
    const tempCorrection =
      NEC_TEMPERATURE_CORRECTION_FACTORS[temperatureRating];
    const tempFactor = tempCorrection
      ? tempCorrection[ambientTemperature as keyof typeof tempCorrection] || 1.0
      : 1.0;

    return NEC_WIRE_GAUGES.filter(
      (wire) => awgToNumber(wire.awg) <= awgToNumber(maxWireSize),
    ).map((wire) => {
  ...
  const summaryStats = useMemo(() => {
    ...
    const avgDerating =
      chartData.reduce((sum, wire) => sum + wire.totalDerating, 0) / totalWires;
    const maxCapacity = Math.max(
      ...chartData.map((wire) => wire.adjustedCapacity),
    );
    ...
    <TableContainer component={Paper}>
      <Table size="small">
        <TableHead>
          <TableRow>
            <TableCell>
              <strong>AWG Size</strong>
            </TableCell>
            <TableCell align="right">
              <strong>Area (kcmil)</strong>
            </TableCell>
            <TableCell align="right">
            ...
            <TableCell align="right">
              <strong>Resistance (Ω/1000ft)</strong>
            </TableCell>
            <TableCell align="right">
            ...
            <TableRow key={wire.awg} hover>
              ...
              <TableCell align="right">
                <Chip
                  ...
                  size="small"
                  color={
                    wire.totalDerating > 0.8
                      ? "success"
                      : wire.totalDerating > 0.6
                        ? "warning"
                        : "error"
                  }
                />
    ...
    <Box>
        ...
        <Grid item xs={12} md={3}>
          <FormControl fullWidth size="small">
            ...
            <Select
              ...
              onChange={(e) =>
                setSelectedConductorCount(e.target.value as number)
              }
            >
      ...
      <Card>
        <CardContent>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Current carrying capacity adjusted for insulation type (
            {insulationType}), conductor grouping ({selectedConductorCount}{" "}
            conductors), and ambient temperature ({ambientTemperature}°C).
          </Typography>
            ...
            <Grid item xs={12} md={4}>
              <Paper sx={{ p: 2, textAlign: "center" }}>
                <Typography variant="h6" color="secondary">
                  {(
                    (NEC_TEMPERATURE_CORRECTION_FACTORS[temperatureRating]?.[
                      ambientTemperature as keyof (typeof NEC_TEMPERATURE_CORRECTION_FACTORS)[typeof temperatureRating]
                    ] || 1.0) * 100
                  ).toFixed(0)}
                  %
                </Typography>
      ...
      <Card sx={{ mt: 3 }}>
        <CardContent>
          ...
          {NEC_INSULATION_TYPES[
            insulationType as keyof typeof NEC_INSULATION_TYPES
          ] && (
            <Box>
