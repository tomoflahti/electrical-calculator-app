...
const generateVoltageDropGraphData = (params: {
  ...
  const lengths = Array.from(
    { length: 21 },
    (_, i) => (i * params.maxDistance) / 20,
  );
  return lengths.map((distance) => ({
    distance,
    voltageDropPercent:
      ((2 * params.current * distance * 0.0002) / params.voltage) * 100, // Simplified calculation
...
const VoltageDropChart: React.FC<VoltageDropChartProps> = ({
  ...
  const [selectedMetric, setSelectedMetric] = React.useState<
    "voltageDropPercent" | "powerLossWatts" | "efficiencyPercent"
  >("voltageDropPercent");

  // Generate data for multiple cable sizes
  const chartData = useMemo(() => {
    const allData: Record<number, { distance: number; [key: string]: number }> =
      {};
    ...
    cableSizes.forEach((cableSize) => {
      ...
      graphData.forEach((point) => {
        ...
        allData[point.distance][`${cableSize}mm²`] =
          point[selectedMetric as keyof typeof point];
      });
    });

    return Object.values(allData).sort((a, b) => a.distance - b.distance);
  }, [
    cableSizes,
    current,
    voltage,
      ...
      <CardContent>
        ...
        <Box mt={2}>
          <Typography variant="body2" color="text.secondary">
            Current: {current}A • Voltage: {voltage}V • Power Factor:{" "}
            {powerFactor} • Material: {conductorMaterial}
          </Typography>
          {selectedMetric === "voltageDropPercent" && showVoltageLimits && (
            <Typography variant="body2" color="text.secondary">
