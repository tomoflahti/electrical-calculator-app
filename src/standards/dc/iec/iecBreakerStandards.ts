...
export function getPrimaryIECBreakerRecommendation(
  ...
  return specifications.sort((a, b) => {
    ...
    if (
      ["solar", "industrial"].includes(applicationType) &&
      a.standard === "IEC60947" &&
      b.standard !== "IEC60947"
    )
      return -1;
    ...
    if (
      ...
      a.standard === "IEC60898-3" &&
      b.standard !== "IEC60898-3"
    )
      return -1;
    if (
      applicationType === "solar" &&
 ...
 */
export function getAvailableIECBreakerRatings(
  applicationType: DCApplicationType,
): number[] {
  const applicableBreakers = IEC_BREAKER_SPECIFICATIONS.filter((breaker) =>
    breaker.applications.includes(applicationType),
  );
