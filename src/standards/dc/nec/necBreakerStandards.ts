...
export function getPrimaryNECBreakerRecommendation(
  ...
  return specifications.sort((a, b) => {
    if (applicationType === 'marine' && a.standard === 'ABYC' && b.standard !== 'ABYC') return -1;
    if (applicationType === 'marine' && a.standard !== 'ABYC' && b.standard === 'ABYC') return 1;
    
    if (applicationType === 'automotive' && a.standard === 'SAE' && b.standard !== 'SAE') return -1;
    if (applicationType === 'automotive' && a.standard !== 'SAE' && b.standard === 'SAE') return 1;
 ...
 */
export function getAvailableNECBreakerRatings(applicationType: DCApplicationType): number[] {
  const applicableBreakers = NEC_BREAKER_SPECIFICATIONS.filter(
