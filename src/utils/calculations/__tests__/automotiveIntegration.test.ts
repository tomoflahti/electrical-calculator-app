...
describe('Automotive Fuse Integration with DC Breaker Router', () => {
  describe('Router Logic - Automotive Fuse vs Circuit Breaker Selection', () => {
    ...
    it('should route to automotive fuse calculator for 24V commercial applications', () => {
      ...
      expect(result.standard).toBe('ISO8820-3');
      expect(result.safetyFactor).toBe(1.30); // 24V safety factor
    ...
    it('should route to automotive fuse calculator for 48V hybrid applications', () => {
      ...
      expect(result.standard).toBe('ISO8820-3');
      expect(result.safetyFactor).toBe(1.20); // 48V safety factor
  ...
  describe('Standards Switching', () => {
    it('should use automotive fuse calculation regardless of wire standard', () => {
      ...
      expect(necResult.recommendedBreakerRating).toBe(iecResult.recommendedBreakerRating);
    });
  ...
  describe('Validation Integration', () => {
    ...
    it('should validate temperature limits for automotive fuses', () => {
      ...
      const errors = validateDCBreakerInput(invalidInput);
      expect(errors.length).toBeGreaterThan(0);
      expect(errors[0]).toContain('Ambient temperature must be between -40°C and 85°C');
    });
    ...
    it('should validate power-based inputs for automotive applications', () => {
      ...
      const errors = validateDCBreakerInput(invalidInput);
      expect(errors.length).toBeGreaterThan(0);
      expect(errors[0]).toContain('Calculated current may exceed automotive fuse range');
    });
  ...
  describe('Real-World Integration Scenarios', () => {
    it('should handle complete 12V passenger car electrical system', () => {
      const scenarios = [
        { component: 'Interior LED', power: 60, expectedFuse: 10 },
        { component: 'Headlights', power: 144, expectedFuse: 20 },
        { component: 'Cooling Fan', power: 288, expectedFuse: 40 },
        { component: 'Audio System', power: 240, expectedFuse: 30 }
      ];
    ...
    it('should handle 48V hybrid vehicle electrical system', () => {
      const scenarios = [
        { component: 'Auxiliary Systems', power: 240, expectedFuse: 10 },
        { component: 'Electric Heater', power: 1200, expectedFuse: 35 },
        { component: 'High-Power Audio', power: 960, expectedFuse: 30 }
      ];
    ...
    it('should handle marine electrical systems', () => {
      const scenarios = [
        { component: 'Navigation Lights', power: 96, voltage: 12, expectedFuse: 15 },
        { component: 'Bilge Pump', power: 180, voltage: 12, expectedFuse: 25 },
        { component: 'Cabin Lighting', power: 144, voltage: 24, expectedFuse: 10 }
      ];
