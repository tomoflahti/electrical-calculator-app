 ...
 */
export function calculateAutomotiveFuseSize(input: DCBreakerCalculationInput): DCBreakerCalculationResult {
  const {
    inputMethod,
    loadCurrent,
    loadPower,
  ...
  if (![12, 24, 32, 48].includes(automotiveVoltage)) {
    throw new Error(`Unsupported automotive voltage: ${automotiveVoltage}V. Supported voltages: 12V, 24V, 32V, 48V`);
  }

  // Calculate base current
  let baseCurrent: number;
  let powerAnalysis: any = undefined;

  if (inputMethod === 'current') {
    if (!loadCurrent || loadCurrent <= 0) {
      throw new Error('Load current must be greater than 0 for current input method');
    }
    ...
    const efficiency = efficiencyFactor || getAutomotiveEfficiency(automotiveVoltage);
    baseCurrent = calculateAutomotiveCurrent(loadPower, automotiveVoltage, efficiency);
  ...
  const safetyFactor = continuousOperation ? safetyFactors.continuous : safetyFactors.intermittent;
  ...
  if (ambientTemperature > 40) {
    temperatureDerating = calculateAutomotiveTemperatureDerating(ambientTemperature);
  ...
  if (adjustedCurrent > 120) {
    throw new Error(`Adjusted current (${adjustedCurrent.toFixed(1)}A) exceeds automotive fuse range (120A max). Use circuit breaker instead.`);
  }

  // Find appropriate fuse rating
  const recommendedFuseRating = getNextAutomotiveFuseRating(adjustedCurrent, automotiveVoltage, applicationType);
  if (!recommendedFuseRating) {
    throw new Error(`No suitable automotive fuse found for ${adjustedCurrent.toFixed(1)}A at ${automotiveVoltage}V`);
  }

  // Get fuse specifications
  const primaryFuse = getPrimaryAutomotiveFuseRecommendation(recommendedFuseRating, automotiveVoltage, applicationType);
  if (!primaryFuse) {
    throw new Error(`No automotive fuse specification found for ${recommendedFuseRating}A at ${automotiveVoltage}V`);
  }

  const alternativeFuses = getAutomotiveFuseSpecifications(recommendedFuseRating, automotiveVoltage, applicationType)
  ...
  const compliance = {
    ...
    wireCompatible: wireGauge ? checkWireCompatibility(wireGauge, recommendedFuseRating) : true,
    temperatureCompliant: ambientTemperature >= primaryFuse.temperatureRange.min && 
  ...
  return {
    ...
    availableBreakerSizes: getAvailableAutomotiveFuseRatings(automotiveVoltage, applicationType),
 ...
 */
function calculateAutomotiveTemperatureDerating(ambientTemperature: number): number {
 ...
 */
function checkWireCompatibility(wireGauge: string, fuseRating: number): boolean {
 ...
 */
export function validateAutomotiveFuseInput(input: DCBreakerCalculationInput): string[] {
  const errors: string[] = [];

  // Validate voltage system
  const voltage = input.systemVoltage || 12;
  if (![12, 24, 32, 48].includes(voltage)) {
    errors.push(`Unsupported automotive voltage: ${voltage}V. Supported: 12V, 24V, 32V, 48V`);
  }

  // Validate application type
  if (!['automotive', 'marine', 'led'].includes(input.applicationType)) {
    errors.push('Automotive fuses are only supported for automotive, marine, and LED applications');
  }
  ...
  if (input.inputMethod === 'current') {
    ...
    if (input.loadPower && input.systemVoltage) {
      ...
      if (estimatedCurrent > 100) { // Leave room for safety factors
        errors.push('Calculated current may exceed automotive fuse range (120A max)');
      }
    }
  }

  // Validate temperature range
  if (input.ambientTemperature && (input.ambientTemperature < -40 || input.ambientTemperature > 85)) {
    errors.push('Ambient temperature must be between -40°C and 85°C for automotive fuses');
  }
