...
import { calculateAutomotiveFuseSize, validateAutomotiveFuseInput } from './automotive/automotiveFuseCalculator';

/**
 * Main DC circuit breaker calculation function
 * Routes to appropriate engine based on wire standard and application
 */
export function calculateDCBreakerSize(input: DCBreakerCalculationInput): DCBreakerCalculationResult {
...
function shouldUseAutomotiveFuse(input: DCBreakerCalculationInput): boolean {
  ...
  } else if (input.inputMethod === 'power' && input.loadPower && input.systemVoltage) {
    // Account for automotive efficiency factors
    const efficiency = getAutomotiveEfficiencyEstimate(systemVoltage as 12 | 24 | 32 | 48);
  ...
  const temperatureFactor = (input.ambientTemperature && input.ambientTemperature > 40) ? 1.1 : 1.0;
  
  const totalSafetyFactor = baseSafetyFactor * environmentalFactor * temperatureFactor;
  const adjustedCurrent = estimatedCurrent * totalSafetyFactor;
  
  // Use automotive fuse only if current is safely within range (≤100A adjusted to leave room for fuse selection)
  return adjustedCurrent <= 100;
}
...
function getAutomotiveEfficiencyEstimate(voltage: 12 | 24 | 32 | 48): number {
  ...
  const efficiencyMap = {
    ...
    24: 0.90,  // 24V systems are more efficient
 ...
 */
export function validateDCBreakerInput(input: DCBreakerCalculationInput): string[] {
  const errors: string[] = [];

  // Check if automotive fuse validation should be used
  if (shouldUseAutomotiveFuse(input)) {
    return validateAutomotiveFuseInput(input);
  }
  ...
  if (input.ambientTemperature) {
    const minTemp = -40;
    const maxTemp = ['automotive', 'marine', 'led'].includes(input.applicationType) ? 85 : 150;
    if (input.ambientTemperature < minTemp || input.ambientTemperature > maxTemp) {
      if (maxTemp === 85) {
  ...
  if (input.applicationType === 'solar') {
    if (!input.shortCircuitCurrent && (!input.panelIsc || !input.numberOfPanels)) {
      errors.push('Solar applications require either shortCircuitCurrent or panelIsc with numberOfPanels');
    }
  }

  // Application type validation
  const validApplications = ['automotive', 'marine', 'solar', 'telecom', 'battery', 'led', 'industrial'];
