...
import type { ConduitSpecification, WireSpecification } from '../../types/standards';

/**
 * Calculate conduit fill using IEC/Metric standards (New unified interface)
 */
export function calculateIECConduitFill(input: ConduitFillCalculationInput): ConduitFillCalculationResult;

/**
 * Calculate conduit fill using IEC/Metric standards (Legacy interface)
 */
export function calculateIECConduitFill(input: IECConduitFillInput): IECConduitFillResult[];

/**
 * Calculate conduit fill using IEC/Metric standards (Implementation with overloads)
 */
export function calculateIECConduitFill(input: ConduitFillCalculationInput | IECConduitFillInput): ConduitFillCalculationResult | IECConduitFillResult[] {
 ...
 */
function calculateIECConduitFill_New(input: ConduitFillCalculationInput): ConduitFillCalculationResult {
  const {
    wires,
    conduitType,
    conduitSize,
  ...
  const availableConduits = IEC_CONDUIT_SPECIFICATIONS
    .filter(c => c.type === conduitType)
  ...
  if (conduitSize) {
    const specificConduit = availableConduits.find(c => c.size === conduitSize);
    if (specificConduit) {
  ...
  for (const conduit of availableConduits) {
    const iecFillData = calculateIECFillData(conduit, adjustedWireArea, totalWireCount);
  ...
  if (!recommendedConduit) {
    ...
    const iecFillData = calculateIECFillData(recommendedConduit, adjustedWireArea, totalWireCount);
    fillPercentage = iecFillData.fillPercentage;
    maxAllowedFillPercentage = iecFillData.maxAllowedFillPercentage;
    fillRule = iecFillData.fillRule;
  }
  ...
  const applicationRequirements = getApplicationRequirements(applicationType, 'IEC');
  const installationFactors = getInstallationMethodFactors(installationMethod, 'IEC');
 ...
 */
function calculateIECWireBreakdown(wires: ConduitFillCalculationInput['wires']): ConduitFillCalculationResult['wireBreakdown'] {
 ...
 */
function getIECWireSpecification(gauge: string, insulation: string): WireSpecification | undefined {
  const wireTable = insulation === 'XLPE' 
    ? IEC_XLPE_WIRE_SPECIFICATIONS 
    : IEC_WIRE_SPECIFICATIONS;
...
function calculateIECFillData(
  ...
  const maxAllowableArea = (conduit.internalArea * maxAllowedFillPercentage) / 100;
...
function calculateSpecificIECConduitFill(
  ...
  const applicationRequirements = getApplicationRequirements(input.applicationType, 'IEC');
  const installationFactors = getInstallationMethodFactors(input.installationMethod, 'IEC');
  ...
  return {
    ...
    fillAnalysis: {
      ...
      fillRule: iecFillData.fillRule as '53% (1 wire)' | '31% (2 wires)' | '40% (3+ wires)'
    },
 ...
 */
export function getIECConduitSpecifications(conduitType?: string): ConduitSpecification[] {
  if (conduitType) {
 ...
 */
export function getIECWireSpecifications(insulationType: 'PVC' | 'XLPE' = 'PVC'): WireSpecification[] {
  return insulationType === 'XLPE' ? IEC_XLPE_WIRE_SPECIFICATIONS : IEC_WIRE_SPECIFICATIONS;
}
...
export function getIECBundlingFactor(numberOfCables: number): number {
  const counts = Object.keys(IEC_BUNDLING_FACTORS).map(Number).sort((a, b) => a - b);
...
export function generateIECConduitFillComparisonData(
  ...
  const conduits = IEC_CONDUIT_SPECIFICATIONS.filter(c => c.type === conduitType);
 ...
 */
export function calculateIECConduitFill_Legacy(input: IECConduitFillInput): IECConduitFillResult[] {
  ...
  const availableConduits = IEC_CONDUIT_SPECIFICATIONS.filter(c => c.type === input.conduitType);
  ...
  if (totalWireCount === 0) {
    ...
    return results.sort((a, b) => parseFloat(a.conduitSize) - parseFloat(b.conduitSize));
  }

  for (const conduit of availableConduits) {
    const iecFillData = calculateIECFillData(conduit, totalWireArea, totalWireCount);
    
    // Calculate max wires for this conduit (avoid division by zero)
    const maxWires = totalWireCount > 0 
      ? Math.floor((iecFillData.maxAllowedFillPercentage * conduit.internalArea / 100) / (totalWireArea / totalWireCount))
      : 0;
    ...
    results.push({
      ...
      isCompliant: iecFillData.isCompliant,
      availableArea: (conduit.internalArea * iecFillData.maxAllowedFillPercentage) / 100,
      usedArea: totalWireArea,
 ...
 */
export function findRecommendedConduitSize(input: IECConduitFillInput): IECConduitFillResult | null {
