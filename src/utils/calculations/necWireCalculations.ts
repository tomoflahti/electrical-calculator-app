...
const NEC_INSTALLATION_METHODS: Record<string, NECInstallationMethodFactor> = {
  ...
  direct_burial: {
    ...
    applicableConditions: [
      "Thermal resistivity considerations",
      "Moisture protection",
    ],
  },
  free_air: {
 ...
 */
export function calculateNECWireSize(
  ...
  if (suitableWires.length === 0) {
    throw new Error(
      "No suitable NEC wire size found for the given ampacity requirements",
    );
  }
  ...
  // Compliance checks
  const finalAmpacity = getAmpacityForTemperature(
    selectedWire,
    temperatureRating,
  );
  ...
  const installationCompliant =
    NEC_INSTALLATION_METHODS[installationMethod] !== undefined;
...
function calculateNECCorrectionFactors({
  ...
  const conductorKeys = Object.keys(NEC_CONDUCTOR_ADJUSTMENT)
    .map(Number)
    .sort((a, b) => a - b);
  ...
  if (numberOfConductors > conductorKeys[conductorKeys.length - 1]) {
    groupingFactor =
      NEC_CONDUCTOR_ADJUSTMENT[conductorKeys[conductorKeys.length - 1]];
  }

  // Installation method factor
  const installationFactor =
    NEC_INSTALLATION_METHODS[installationMethod]?.factor || 1.0;
...
function calculateNECVoltageDropForWire(
  ...
  if (voltageSystem === "single") {
    ...
    voltageDropVolts =
      (Math.sqrt(3) * current * length * resistance * powerFactor) / 1000;
  }
 ...
 */
function getAmpacityForTemperature(
  wire: NECWireSpecification,
  temperatureRating: number,
): number {
