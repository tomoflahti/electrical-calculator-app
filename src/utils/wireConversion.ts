...
export function getMetricWireSizes(): number[] {
  return [1.5, 2.5, 4, 6, 10, 16, 25, 35, 50, 70, 95, 120, 150, 185, 240, 300, 400, 500];
}
...
export function findMinimumMetricWireSize(requiredMm2: number): number {
  ...
  return metricSizes.find(size => size >= requiredMm2) || metricSizes[metricSizes.length - 1];
}

/**
 * Convert AWG wire specification to metric equivalent
 */
export function convertAwgWireToMetric(awgWire: any): any {
 ...
 */
export function formatWireSize(wireSize: string, standard: 'NEC' | 'IEC'): string {
